...
AddEventHandler("Drugs:Client:Startup", function()
	plsr.State.flags.drugsRunSpeed = false
end)
...
RegisterNetEvent("Drugs:Effects:RunSpeed", function(quality)
    ...
    local addiction = plsr.State.character.Addiction?.Coke?.Factor or 0.0
    ...
    plsr.State.flags.drugsRunSpeed = true
    StartScreenEffect("DrugsMichaelAliensFight", 3.0, 0)
	SetRunSprintMultiplierForPlayer(pId, total)
	SetSwimMultiplierForPlayer(pId, total)

    StatSetInt(`MP0_STAMINA`, 100, true)
	plsr.Buffs:ApplyUniqueBuff("speed", _runSpeedTime, false)
	while _runSpeedTime > 0 and not plsr.State.flags.isDead do
    ...
    plsr.Buffs:RemoveBuffType("speed")
...
RegisterNetEvent("Drugs:Effects:Armor", function(quality)
    ...
    local addiction = plsr.State.character.Addiction?.Meth?.Factor or 0.0
	...
	local loops = 0
	plsr.Buffs:ApplyUniqueBuff("armor", _armorTime, false)
	while _armorTime > 0 and not plsr.State.flags.isDead do
		loops = loops + 1
		Wait(1000)
...
RegisterNetEvent("Drugs:Effects:Heal", function(quality)
    ...
    local addiction = plsr.State.character.Addiction?.Moonshine?.Factor or 0.0
	...
	plsr.Status.Modify:Add("PLAYER_DRUNK", math.ceil(10 * (1.0 + (drugEffectQuality / 100))))
	_healTime = math.ceil(30 * (1.0 + (drugEffectQuality / 100)) * (1.0 - (addiction / 100)))
	local loops = 0
	plsr.Buffs:ApplyUniqueBuff("heal", _healTime, false)
	while _healTime > 0 and not plsr.State.flags.isDead do
		local ped = PlayerPedId()
		loops = loops + 1
		Wait(1000)
...
RegisterNetEvent("Characters:Client:Spawned", function()
    plsr.Buffs:RegisterBuff("speed", "bolt-lightning", "#8419C2", -1, "timed")
    plsr.Buffs:RegisterBuff("armor", "shield-plus", "#4056b3", -1, "timed")
    plsr.Buffs:RegisterBuff("heal", "trash-plus", "#52984a", -1, "timed")
end)
